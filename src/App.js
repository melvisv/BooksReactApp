...
import {Table, FormGroup, Label, Input, Button, Modal, ModalHeader, ModalBody, ModalFooter } from 'reactstrap';
...
class App extends Component
{
  ...
  componentDidMount() {
    axios.get('http://localhost:62523/book').then((response) => {
        this.setState({
          books: response.data
        }) 
    });
  ...
  deleteBook(id ){
    ...
    axios.delete('http://localhost:62523/book/delete/' + id).then((response) => {
        this._refreshBooks();
  });
}
...
_refreshBooks() {
  ...
  addBook() {
    axios.post('http://localhost:62523/book' , this.state.newBookData).then((response) => {
  ...
  updateBook(){
    ...
    axios.put('http://localhost:62523/book/update/' + this.state.editBookData.id,  
    {    
      title, rating  
    }
    ).then((response) => {
        this._refreshBooks();
        this.state.editBookModal = !this.state.editBookModal;
